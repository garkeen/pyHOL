...
> 代码事实以 `kernel/macro.py`、`kernel/theory.py`（宏注册部分）、`logic/macros/core.py` 为准。
...
注册在 `logic/macros/core.py`。最常用的宏：
...
`logic.logic.apply_theorem(th_name, *pts, concl=None, inst=None)` 是包装函数：无 `concl`/`inst` 时用 `apply_theorem` 宏，否则用 `apply_theorem_for`。

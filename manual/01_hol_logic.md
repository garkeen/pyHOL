...
> 本章是高阶逻辑（Higher-Order Logic, HOL）的纯理论介绍，不涉及任何 pyHOL 代码。后续章节在此基础上引入 kernel 实现。
...
为了快速判断两个项是否 α-等价，pyHOL 使用 **de Bruijn 索引**表示绑定变量。绑定变量在原则上无名，引用时用 `Bᵢ`，其中 `i` 是从引用位置到绑定它的 λ 的"深度"（从 0 开始数中间隔了多少层 λ）。

例如：
- `λx. x + 2` 表示为 `λ_. B₀ + 2`（`x` 的引用与绑定它的 λ 之间隔 0 层）。
- `λx. λy. x + y` 表示为 `λ_. λ_. B₁ + B₀`（`x` 隔 1 层，`y` 隔 0 层）。

在 pyHOL 中，`Abs` 的 `var_name` 字段只是**建议名**（用于打印和发明新变量时的起点），`var_T` 是**有意义的**（不同类型给出不同的项），`body` 中的绑定变量用 `Bound(n)` 引用。
...
高阶逻辑的推理规则由一组**原始推理规则**（primitive deduction rules）定义。这些规则是逻辑的根基，是唯一能"凭空"构造定理的入口。pyHOL 的内核实现了 15 条原语（见 `02_kernel.md`）。此处先介绍其逻辑含义。
...
不含 `falseE` 与 `classical` 的子系统是直觉主义逻辑：只能证明构造性成立的命题。加入排中律 `classical` 后得到经典逻辑，可证明所有语义有效的命题。pyHOL 默认采用经典逻辑（`classical` 是公理）。
...
**下一章**：[`02_kernel.md`](02_kernel.md) -- pyHOL 如何用 Python 实现上述类型、项、定理与 15 条原语。

"""Omega Test decision procedure -- pure algorithm core.
...
This module is the factoid / solver core only: it works on coefficient
tuples and derivation trees and never constructs a proof.  The proof
assembly and auto registration live in theories/integer/omega.py
(audit 3 dependency law: solvers must not import theories).
"""

from syntax import numeral
from syntax.numeral import strip_plus
import collections
import functools
import copy
...
def term_to_factoid(vars, t):
    ...
    return Factoid(mk_coeff(vars, strip_plus(t.arg)))
...
def solve(em, db, width):
    ...
    elif all(df.factoid.is_zero_var_factoid()
             for dfs in db.values() for df in dfs):
        # Constant-only system: no variable occurs in any factoid, so the
        # only decision is whether some factoid is trivially false.  This
        # must be handled before the elimination branch below, where
        # exact_var / least_coeff_var both return None (nothing to
        # eliminate) and the code would index a factoid with None.
        for dfs in db.values():
            for df in dfs:
                if df.factoid.is_false_factoid():
                    return mode_result(em, Contr(df.deriv))
        return mode_result(em, Satisfiable(dict()))

...
class ThmTest(unittest.TestCase):
    ...
    def testCheckCanProve(self):
        test_data = [
            ...
            (Thm(A, A), Thm(A, (A, B)), True),
            (Thm(A, A), Thm(A), False),
            (Thm(A, A), Thm(B, A), False),
        ]
    ...
    def testImpliesIntr2(self):
        th = Thm(B, (A, B))
        self.assertEqual(Thm.implies_intr(A, th), Thm(Implies(A,B), B))
    ...
    def testSymmetric(self):
        th = Thm(Eq(x,y), A)
        self.assertEqual(Thm.symmetric(th), Thm(Eq(y,x), A))
    ...
    def testTransitive(self):
        th1 = Thm(Eq(x,y), A)
        th2 = Thm(Eq(y,z), B)
        self.assertEqual(Thm.transitive(th1, th2), Thm(Eq(x,z), (A, B)))
    ...
    def testCombination(self):
        th1 = Thm(Eq(f,g), A)
        th2 = Thm(Eq(x,y), B)
        self.assertEqual(Thm.combination(th1, th2), Thm(Eq(f(x),g(y)), (A, B)))
    ...
    def testEqualIntr(self):
        th1 = Thm(Implies(A,B), A)
        th2 = Thm(Implies(B,A), B)
        self.assertEqual(Thm.equal_intr(th1, th2), Thm(Eq(A,B), (A, B)))
    ...
    def testEqualElim(self):
        th1 = Thm(Eq(A,B), A)
        th2 = Thm(A, B)
        self.assertEqual(Thm.equal_elim(th1, th2), Thm(B, (A, B)))

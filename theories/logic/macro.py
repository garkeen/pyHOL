...
def imp_conj_proof(goal, pts):
    """Proof logic of imp_conj_macro: goal is of the form
    A_1 & ... & A_m --> B_1 & ... & B_n, where {A_1, ..., A_m} is a
    subset of {B_1, ..., B_n}."""
    dct = dict()
...
class imp_conj_macro(Macro):
    ...
    def get_proof_term(self, goal, pts):
        return imp_conj_proof(goal, pts)


def imp_disj_proof(goal, pts):
    """Proof logic of imp_disj_macro: goal is of the form
    A_1 | ... | A_m --> B_1 | ...| B_n, where {A_1, ..., A_m} is a
    subset of {B_1, ..., B_n}."""
...
class imp_disj_macro(Macro):
    ...
    def get_proof_term(self, goal, pts):
        return imp_disj_proof(goal, pts)


def resolution_proof(arg, pts):
    """Proof logic of resolution_macro: input proof terms are
    A_1 | ... | A_m and B_1 | ... | B_n, where there is some i, j such
    that B_j = ~A_i or A_i = ~B_j."""
    ...
    if side == 'right':
        return resolution_proof(arg, [pt2, pt1])
...
class resolution_macro(Macro):
    ...
    def get_proof_term(self, arg, pts):
        return resolution_proof(arg, pts)

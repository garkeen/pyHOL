...
- **真实定理闭环验证** `theories/tests/z3rec_real_theorems.py`：
...
| 工具 | 内容 | 结果 |
| --- | --- | --- |
| `theories/tests/z3rec_corpus.py` | 32 目标回归 runner；每目标独立子进程 + 20s 硬杀 + 全局预算 + xfail 记账 | 28 PASS + 4 XFAIL |
| `theories/tests/z3rec_smoke_test.py` | 10 目标冒烟，断言 rule≠sorry、无 gap、**核级 check_proof** | 10/10 |
| `theories/tests/z3rec_unit_test.py` | 6 个 z3-free 单元测试（occurs/数值求值/原子网/条件式 schematic/命题网） | 6/6 |
| `theories/tests/z3rec_real_theorems.py` | 真实定理端到端闭环 | 9 CLOSED + 1 XFAIL |
...
```bash
# 语料回归（仓库根目录）
PROOFREC_TIMEOUT=20 python theories/tests/z3rec_corpus.py [类别...]
# 真实定理闭环
PROOFREC_TIMEOUT=40 python theories/tests/z3rec_real_theorems.py
...
- Sascha Böhme and Tjark Weber, "Fast LCF-Style Proof Reconstruction for Z3"
  ...
  权威描述、代码头注释（`theories/z3rec.py`）亦保留引用，存档正文不再保留，
  需要时按标题自行检索论文。

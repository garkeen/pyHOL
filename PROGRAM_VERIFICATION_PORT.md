...
- **9 条重放里 6 条 VALID**（含 `_exhaustive`/`_cases`/`_induct`）；失败的是两条**递归方程**
  ...
  但注意换收口步没用（试过 `resolve`，同一形状照旧差一条），得先按 §5.8 第 53 条末尾的
  受控测量把「收口步何时落新行」的规则搞清楚，再让模板把那个条件**摆出来**；
  修完这两条方程应当直接过，投影（每函数 `def` + 方程定理 + 四条规则的投影）也可以照着
  同一个模式做。
...
53. **发射器的 ID 算术在同头嵌套模式的反驳上会差一条**（`core/fungen.py` 的
    ...
    修的方向**不是**换一个收口步：试过把收口换成 `← resolve <不相交性定理> goal=<false>
    facts=[<等式事实>]`（连翻转那步一起），结果同一形状照旧差一条——`resolve` 实测有时落一条
    有时不落（`false` 行的假设集合是否已存在决定去重）。**根因在状态层**：任何「用事实关掉一个
    已存在行」的收口步（`rule ... facts=[...]`、`resolve`）落不落新行取决于结果 Thm 的
    （假设集合 + 命题）是否已在表里，而这是发射器静态算不出来的。
    下一步（未做）：先做一次受控测量——在 REPL 里对同一 `false` 行分别用「事实是行自己的假设」
    与「事实是派生行」两种收口，打印结果 Thm 的假设集合，把规则搞清楚；然后二选一：
    (i) 让模板的收口步恒落新行（例如把要用的等式先 `cut` 成一条**独立的新行**，收口时引用它）；
    (ii) 让收口前的行「必然不在表里」（例如收口针对的行是模板刚 `intro` 出来的、且其假设集合
    与已有的都不同——需要先把测量结论拿到手）。
    和类型的编码正撞在这条上（每条边两个方程共享构造子，`Left (Suc n)` 要反驳 `Left 0`），
    修完才能发射编码后的定义。

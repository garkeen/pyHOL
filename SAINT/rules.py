...
from SAINT.expr import Var, Const, Fun, EvalAt, Op, Integral, Symbol, Expr, \
...
from SAINT.solve import solve_equation, solve_for_term
from SAINT import latex
from SAINT import limits
from SAINT import norm
from SAINT.context import Context
from SAINT import poly
from SAINT.poly import from_poly, to_poly, normalize
...
def _expand_ones(e: Expr) -> Expr:
    """Wrap single factors inside sums as explicit 1 * factor.

    Used only as a fallback for matching integral identities, so that a
    body like x / (x ^ 2 + 1) can match a pattern x / (?a * x ^ 2 + ?b)
    with a = 1.  This is deliberately local to DefiniteIntegralIdentity:
    the general matcher does not treat a power as a product.

    """
    if e.is_plus():
        return Op('+', _expand_ones(_wrap_one(e.args[0])), _expand_ones(_wrap_one(e.args[1])))
    elif e.is_op() and len(e.args) == 2:
        return Op(e.op, _expand_ones(e.args[0]), _expand_ones(e.args[1]))
    elif e.is_fun():
        return expr.Fun(e.func_name, *(_expand_ones(a) for a in e.args))
    elif isinstance(e, IndefiniteIntegral):
        return IndefiniteIntegral(e.var, _expand_ones(e.body), e.skolem_args)
    else:
        return e


def _wrap_one(t: Expr) -> Expr:
    t = _expand_ones(t)
    if t.is_plus() or t.is_times():
        return t
    return Op('*', Const(1), t)


def _match_indefinite(var: str, body: Expr, lhs: Expr):
    """Match an integral body against an identity lhs, with a fallback
    that treats single factors as 1 * factor."""
    target = IndefiniteIntegral(var, body, tuple())
    inst = expr.match(target, lhs)
    if inst is not None:
        return inst
    target2 = IndefiniteIntegral(var, _expand_ones(body), tuple())
    return expr.match(target2, _expand_ones(lhs))
...
class DefiniteIntegralIdentity(Rule):
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        ...
        for identity in ctx.get_indefinite_integrals():
            body2 = _minus_to_plus(e.body)
            inst = _match_indefinite(e.var, body2, identity.lhs)
            if inst is None:
                continue
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        def rec(cur_e, loc, ctx):
            if loc.is_empty():
                ...
                ctx2.set_ambient_var(cur_e.var)
                if loc.head == 0:
...
class FullSimplify(Rule):
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        ...
        while True:
            ...
            s = OnSubterm(ExpandPolynomial()).eval(s, ctx)
            ...
            s = OnSubterm(DefiniteIntegralIdentity()).eval(s, ctx)
            if s == current:
                break
            current = s
            counter += 1
...
def _cancel_fractions(e: Expr, conds: Conditions) -> Expr:
    """Cancel identical numerator/denominator pairs after normalization.

    ``to_poly``/``normalize`` sometimes leave a ratio ``A / A`` behind when the
    shared factor is not known to be nonzero from the conditions (e.g.
    ``sec(4*x)*tan(4*x) / (sec(4*x)*tan(4*x)*4)``).  This performs the
    algebraic cancellation that polynomial division cannot do.

    """
    if e.is_divides():
        num = normalize(e.args[0], conds)
        den = normalize(e.args[1], conds)
        if num == den:
            return Const(1)
        new_num = _cancel_fractions(e.args[0], conds)
        new_den = _cancel_fractions(e.args[1], conds)
        if new_num != e.args[0] or new_den != e.args[1]:
            return _cancel_fractions(Op('/', new_num, new_den), conds)
        return e
    elif e.is_times() or e.is_plus() or e.is_minus():
        if len(e.args) == 1:
            new_arg = _cancel_fractions(e.args[0], conds)
            return Op(e.op, new_arg) if new_arg != e.args[0] else e
        new0 = _cancel_fractions(e.args[0], conds)
        new1 = _cancel_fractions(e.args[1], conds)
        if new0 != e.args[0] or new1 != e.args[1]:
            return Op(e.op, new0, new1)
        return e
    elif e.is_uminus():
        new_arg = _cancel_fractions(e.args[0], conds)
        return Op('-', new_arg) if new_arg != e.args[0] else e
    return e


def _minus_to_plus(e: Expr) -> Expr:
    """Rewrite a - b as a + (-b) to enable pattern matching (e.g. 1/(x-2)
    against the identity 1/(x+a))."""
    if e.is_op():
        if e.op == '-' and len(e.args) == 2:
            return Op('+', _minus_to_plus(e.args[0]), Op('-', _minus_to_plus(e.args[1])))
        return Op(e.op, *[_minus_to_plus(arg) for arg in e.args])
    elif e.is_fun():
        return Fun(e.func_name, *[_minus_to_plus(arg) for arg in e.args])
    elif e.is_deriv():
        return Deriv(e.var, _minus_to_plus(e.body))
    elif e.is_indefinite_integral():
        return IndefiniteIntegral(e.var, _minus_to_plus(e.body), e.skolem_args)
    elif e.is_integral():
        return Integral(e.var, _minus_to_plus(e.lower), _minus_to_plus(e.upper),
                        _minus_to_plus(e.body))
    elif e.is_evalat():
        return EvalAt(e.var, _minus_to_plus(e.lower), _minus_to_plus(e.upper),
                      _minus_to_plus(e.body))
    elif e.is_summation():
        return Summation(e.index_var, _minus_to_plus(e.lower), _minus_to_plus(e.upper),
                         _minus_to_plus(e.body))
    elif e.is_skolem_func():
        return SkolemFunc(e.name, tuple(_minus_to_plus(v) for v in e.dependent_vars))
    return e


def _mono_degree(mono, var: Var) -> int:
    for f, e in mono.factors:
        if f == var:
            if isinstance(e, poly.Polynomial) and e.is_fraction():
                return e.get_fraction()
            return e
    return 0
...
def _mono_div(a, b, conds):
    """Divide monomial a by monomial b, collecting powers of equal bases."""
    factors = {}
    for f, e in a.factors:
        factors[f] = factors.get(f, 0) + _exp_num(e)
    for f, e in b.factors:
        factors[f] = factors.get(f, 0) - _exp_num(e)
    factors = tuple((f, e) for f, e in factors.items() if e != 0)
    return poly.Monomial(a.coeff / b.coeff, factors, conds)


def _poly_long_divide(p1, p2, conds):
    """Long divide p1 / p2 (univariate polynomials) into quotient and remainder.

    Returns ``(quotient, remainder)`` as :class:`poly.Polynomial` if division
    is possible and non-trivial, otherwise None.  The remainder has strictly
    smaller degree than ``p2``.
    """
    if len(p1.monomials) == 0 or len(p2.monomials) == 0:
        return None
    vars_ = set()
    for m in p2.monomials:
        for f, e in m.factors:
            if f.is_var():
                vars_.add(f.name)
    if len(vars_) != 1:
        return None
    var = Var(sorted(vars_)[0])

    def lead(poly):
        return max(poly.monomials, key=lambda m: _mono_degree(m, var), default=None)

    lead_den = lead(p2)
    deg_den = _mono_degree(lead_den, var)
    if deg_den == 0:
        return None
    q = poly.Polynomial([], conds)
    r = p1
    while True:
        lead_r = lead(r)
        if lead_r is None:
            break
        deg_r = _mono_degree(lead_r, var)
        if deg_r < deg_den:
            break
        t = poly.Polynomial([_mono_div(lead_r, lead_den, conds)], conds)
        q = q + t
        r = r - t * p2
        if len(r.monomials) == 0:
            break
        if _mono_degree(lead(r), var) >= deg_r:
            break
    if q.is_fraction() and q.get_fraction() == 0:
        return None
    return q, r


def _subst_power(body: Expr, var: str, var_subst: Expr, new_var: str) -> Expr:
    """Rewrite occurrences of var^n as new_var^(n/k) when var_subst = var^k.

    This handles substitutions where the new variable is a pure power of the
    old one (e.g. u = x^(1/4)), so that x^(3/4) is recognized as u^3 and the
    substitution does not need to solve for x.

    """
    if not (var_subst.is_op() and var_subst.op == '^' and var_subst.args[0] == Var(var)):
        return body
    k = var_subst.args[1]
    if not k.is_const() or not isinstance(k.val, (int, Fraction)):
        return body
    if k.val == 0:
        return body

    replacements = []
    for sub, loc in body.find_subexpr_pred(
            lambda s: s.is_op() and s.op == '^' and s.args[0] == Var(var)):
        n = sub.args[1]
        if not n.is_const() or not isinstance(n.val, (int, Fraction)):
            continue
        m = Fraction(n.val) / Fraction(k.val)
        if m.denominator != 1:
            continue
        if m != 1:  # m == 1 is handled by the ordinary replacement below
            replacements.append((loc, Op('^', Var(new_var), Const(m.numerator))))
    result = body
    for loc, repl in replacements:
        result = result.replace_expr(loc, repl)
    return result
...
class Substitution(Rule):
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        ...
        body = _cancel_fractions(body, ctx2.get_conds())
        body_subst = body.replace_pattern(var_subst, var_name)
        if e.var in body_subst.get_vars() and \
                var_subst.is_op() and var_subst.op == '^' and var_subst.args[0] == Var(e.var):
            # var_subst is a pure power x^k: rewrite every occurrence of
            # x^n as u^(n/k), so e.g. u = x^(1/4) also rewrites x^(3/4)
            # to u^3 without going through the solve branch.
            body_subst = _subst_power(body_subst, e.var, var_subst, self.var_name)
...
class ExpandPolynomial(Rule):
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        ...
        if e.is_power() and e.args[1].is_const() and e.args[1].val > 1 and \
            ...
            div = _poly_long_divide(p1, p2, conds)
            if div is not None:
                q, r = div
                if len(r.monomials) == 0:
                    return from_poly(q)
                return Op('+', from_poly(q), Op('/', from_poly(r), s2))
            if s1 != e.args[0] or s2 != e.args[1]:
                return Op('/', s1, s2)
            return e
        elif e.is_integral():
...
def apply_rewrite_rules(e: Expr, identities, ambient_var: Optional[str] = None, max_depth: int = 20) -> Expr:
    """Apply a list of identities (each with lhs/rhs patterns) to e until fixpoint.

    Each identity is applied directionally (lhs -> rhs).  A subterm matching
    ``lhs`` is replaced by ``rhs`` instantiated with the matched symbols.
    This is the general pattern-matching rewrite engine: the rules themselves
    are data (stored in the context's book), not per-rule code.

    Pattern variables that occur only on the right-hand side of a rule are
    context variables: if ``ambient_var`` is given, they are bound to it.
    For example the identity ``n = n * (sin x)^2 + (cos x)^2`` (TR0) has
    ``x`` occurring only on the rhs, and ``x`` is bound to the variable of
    the enclosing integral.

    Identities marked ``one_shot`` (attribute ``one_shot``) are applied at
    most once, to the first matching subterm; their right-hand side may
    itself match the left-hand side (e.g. introducing ``(sin x)^2+(cos x)^2``
    keeps a constant factor), so fixpoint application would not terminate.

    """
    result = e
    one_shot, rest = [], []
    for identity in identities:
        (one_shot if identity.one_shot else rest).append(identity)

    for identity in one_shot:
        lhs_symbols = set(s.name for s, _ in identity.lhs.find_subexpr_pred(
            lambda s: s.is_symbol()))
        for sub, loc, mapping in expr.find_pattern(result, identity.lhs):
            mapping = dict(mapping)
            if ambient_var is not None:
                for s, _ in identity.rhs.find_subexpr_pred(lambda s: s.is_symbol()):
                    if s.name not in lhs_symbols and s.name not in mapping:
                        mapping[s.name] = expr.Var(ambient_var)
            new_sub = identity.rhs.inst_pat(mapping)
            if new_sub != sub:
                result = result.replace_expr(loc, new_sub)
            break
...
def rewrite_rules_for(ctx: Context, rule_name: str):
    """Return the identities implementing the named rewrite rule.

    Rules are loaded as data from the book (base.calc) using the ``rule``
    field on trigonometric axioms.  No per-rule code is involved.

    """
    return ctx.get_trig_rules(rule_name)


class RewriteTrigonometric(Rule):
    """Rewrite using one of the Fu-style trigonometric rules.

    The rules are data-driven: each rule name maps to a list of identities
    (stored in base.calc with a ``rule`` attribute), applied directionally.
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        return apply_rewrite_rules(e, rewrite_rules_for(ctx, self.rule_name),
                                   ambient_var=ctx.get_ambient_var())


class UnfoldPower(Rule):
    """Unfold powers of expressions."""
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        if e.is_integral():
            return expr.Integral(e.var, e.lower, e.upper, e.body.expand())
        else:
            return e.expand()


class ElimAbs(Rule):
    """Eliminate absolute values by splitting the interval at the point
    where the expression inside changes sign."""

    def __init__(self, c: Optional[Expr] = None):
        self.name = "ElimAbs"
        self.c = c
    ...
    def eval(self, e: Expr, ctx: Context) -> Expr:
        ...
        abs_expr = [sub for sub, _ in e.body.find_subexpr_pred(
            lambda s: s.is_fun() and s.func_name == 'abs')]
        if len(abs_expr) == 0:
            return e
        abs_sub = abs_expr[0]
        arg = abs_sub.args[0]

        # Determine the split point where arg changes sign in (lower, upper).
        if self.c is not None:
            split_pts = [self.c]
        else:
            conds = ctx.get_conds()
            solution = solve_equation(arg, Const(0), e.var, conds)
            split_pts = []
            if solution is not None:
                try:
                    v = expr.eval_expr(solution)
                    if v is not None and (v > 0 or v < 0):
                        split_pts = [solution]
                except NotImplementedError:
                    pass
            if len(split_pts) == 0:
                # No split point found: abs is constant-sign on the interval.
                return e

        def replace_abs(body, sign):
            return body.replace_expr(
                e.body.find_subexpr_pred(
                    lambda s: s == abs_sub)[0][1],
                sign)

        c = split_pts[0]
        result = expr.Integral(e.var, c, e.upper, replace_abs(e.body, arg)) + \
                 expr.Integral(e.var, e.lower, c, replace_abs(e.body, -arg))
        return result
...
def apply_step(current, step, ctx, prev_steps):
    """Apply a single calculation step to the current expression.

    Returns the resulting expression.  Raises AssertionError if the rule
    cannot be applied or produces no change.

    The step's ``location`` (if any) is treated as a hint: it is tried
    first, but if the rule cannot be applied there (no matching subterm,
    no change, or the recorded parameters cannot be reproduced), the rule
    is re-applied to the whole expression and then to each candidate
    subterm.  This makes proofs robust against small differences in
    intermediate forms (e.g. ordering of terms).
    ...
    def make_rule():
        """Construct the rule for this step."""
        if reason == 'Simplification':
            return FullSimplify()
        if reason == 'Substitution':
            return Substitution(step['params']['var_name'], step['params']['g'])
        if reason == 'Integrate by parts':
            return IntegrationByParts(step['params']['parts_u'], step['params']['parts_v'])
        if reason in ('Rewrite', 'Rewrite fraction'):
            return Equation(None, step['params']['rhs'])
        if reason == 'Rewrite trigonometric':
            return RewriteTrigonometric(step['params']['rule'])
        if reason == 'Unfold power':
            return UnfoldPower()
        if reason == 'Elim abs':
            c = (step.get('params') or {}).get('c')
            return ElimAbs(parser.parse_expr(c)) if c is not None else ElimAbs()
        if reason == 'Eliminate infinity':
            return ElimInfInterval()
        if reason == 'Substitution inverse':
            return SubstitutionInverse(step['params']['var_name'], step['params']['g'])
        if reason == 'Split region':
            return SplitRegion(step['params']['c'])
        if reason == 'Solve equation':
            return IntegrateByEquation(prev_steps[int(step['params']['prev_id'])])
        print("Reason: %s" % reason)
        raise NotImplementedError

    def validate(rule, result):
        """Check the recorded parameters are reproduced by the rule."""
        if reason == 'Substitution':
            f = parser.parse_expr(step['params']['f'])
            rule.f = parser.parse_expr(str(rule.f))
            conds = ctx.get_conds()
            if normalize(rule.f, conds) == normalize(f, conds):
                return True
            # The recorded f may use a different (but equivalent) algebraic
            # form, e.g. (15 + 5*u)^-1 vs 1/5 * (1/(u+3)).
            return norm.eq_quotient(rule.f, f, conds) or norm.eq_power(rule.f, f, conds)
        return True

    def changed(res):
        return normalize(res, ctx.get_conds()) != normalize(current, ctx.get_conds())

    def try_at(cur_loc):
        """Apply a fresh rule at cur_loc (or the whole expression if None)."""
        rule = make_rule()
        if cur_loc is None:
            res = rule.eval(current, ctx)
        else:
            res = OnLocation(rule, cur_loc).eval(current, ctx)
        if not validate(rule, res):
            return None
        # Equation/rewrite steps record an explicit target expression: accept
        # the result even if it normalizes back to the current expression
        # (e.g. exp(2*x) -> exp(x)*exp(x) normalizes to the same form).
        if reason in ('Rewrite', 'Rewrite fraction') or changed(res):
            return res
        return None

    # Simplification and similar structural rules may legitimately produce
    # no change (the expression is already in the desired form).
    noop_ok = reason in ('Simplification', 'Elim abs', 'Eliminate infinity')

    # Rules whose lhs is a bare constant pattern variable (TR0) must only be
    # applied at the recorded location: a whole-expression application would
    # match every constant in the expression.
    location_only = reason == 'Rewrite trigonometric' and \
        step.get('params', {}).get('rule') == 'TR0'

    def candidate_locs():
        if reason in ('Substitution', 'Integrate by parts', 'Substitution inverse', 'Split region', 'Elim abs'):
            return [l for _, l in current.separate_integral()]
        if reason == 'Unfold power':
            return [l for _, l in current.find_subexpr_pred(lambda e: e.is_op() and e.op == '^')]
        return []

    first_error = None

    # 1. Try the recorded location (or the whole expression if absent).
    for cur_loc in ([loc] if loc is not None else [None]):
        try:
            res = try_at(cur_loc)
        except (AssertionError, IndexError, KeyError, NotImplementedError) as e:
            first_error = first_error or e
            res = None
        if res is not None:
            return res
        if noop_ok and not loc:
            return current

    # 2. Fallback: try the whole expression.
    ...
    # 3. Fallback: try each candidate subterm.
    for sub_loc in candidate_locs():
        try:
            res = try_at(sub_loc)
        except (AssertionError, IndexError, KeyError, NotImplementedError) as e:
            first_error = first_error or e
    ...
    raise AssertionError("Rule %s does not change the expression" % reason)
...
def check_item(item, target=None, *, debug=False):
    ...
    The .calc format stores only the process (sequence of rules with their
    parameters); the results are re-derived by applying each rule.  A step
    is accepted when the rule can be applied and produces a change.  If
    ...
    for step in item['calc']:
        result = apply_step(current, step, ctx, prev_steps)
        current = result
        prev_steps.append(current)

    if target is not None:
        target = parser.parse_expr(target)
        if normalize(current, ctx.get_conds()) != normalize(target, ctx.get_conds()):

...
from SAINT.expr import *
from SAINT.parser import parse_expr
from SAINT import rules
from SAINT import calc
from SAINT import latex
from SAINT import context
from SAINT.poly import from_poly, to_poly, normalize as poly_normalize
...
class DividePolynomial(AlgorithmRule):
    ...
    def eval(self, e):
        ...
        try:
            new_integral = rules.ExpandPolynomial().eval(e, ctx=ctx)
...
class ElimAbsRule(AlgorithmRule):
    ...
    def eval(self, e, loc=[]):
        ...
        if not abs_exprs:
            return e, None
        result = rule.eval(integ, ctx)
        if result == integ:
            return e, None
        loc = loc if e.ty == INTEGRAL else loc + [1]
        new_problem = e.replace(integ, result)
        c = None
        if result.ty == OP and result.op == "+":
            c = result.args[0].lower
...
class HeuristicIntegrationByParts(HeuristicRule):
    ...
    def eval(self, e, loc=[]):
        ...
        for i in range(len(factors)):
            ...
            if H.ty == INTEGRAL and H.body == h and h != exp(Var(e.var)):
                continue
            u = G
            v = H.body if H.ty == INTEGRAL else H
            try:
...
class HeuristicTrigSubstitution(HeuristicRule):
    ...
    def eval(self, e, loc=None):
        ...
        for s, loc, (a, b) in all_subterms:
            ...
            if a > 0 and b > 0:
                subst = poly_normalize(Op("^", Const(Fraction(a, b)), Const(Fraction(1,2))), ctx.get_conds()) * tan(new_var)
                
            elif a > 0 and b < 0:
                subst = poly_normalize(Op("^", Const(Fraction(a, -b)), Const(Fraction(1,2))), ctx.get_conds()) * sin(new_var)
            
            elif a < 0 and b > 0:
                subst = poly_normalize(Op("^", Const(Fraction(-a, b)), Const(Fraction(1,2))), ctx.get_conds()) * sec(new_var)
...
class HeuristicExpandPower(HeuristicRule):
    ...
    def eval(self, e, loc=[]):
        ...
        for s, l, _ in subexpr:
            base = to_poly(s.args[0], context.Conditions())
            exp = s.args[1].val
...
class AndNode(GoalNode):
    ...
    def compute_value(self):
        ...
        if not self.resolved:
            return self.root
        if len(self.children) == 0:
            return poly_normalize(self.root, ctx.get_conds())
        else:
            for c in self.children:
                value = value.replace(c.root, c.compute_value())
                
            return poly_normalize(value, ctx.get_conds())
...
def perform_steps(node):
    ...
    for step in node.trace():
        ...
        if step.reason == "Simplification":
            ...
            tr_rules = rules.rewrite_rules_for(ctx, step.rule_name)
            if tr_rules:
    ...
    if last_expr.is_constant() and poly_normalize(last_expr, ctx.get_conds()) == last_expr:
        return real_steps

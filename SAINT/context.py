...
class Context:
    ...
    def __init__(self, parent: Optional["Context"] = None):
        ...
        # Named rewrite rules (e.g. trigonometric Fu rules), keyed by rule name.
        # Each is stored in its forward (lhs -> rhs) direction only.
        self.trig_rules: Dict[str, List[Identity]] = dict()
        ...
        # Variable of the enclosing integral/expression, used to bind
        # pattern variables that occur only on the right-hand side of a
        # rewrite rule (e.g. the ``x`` in ``n = n * (sin x)^2 + (cos x)^2``).
        self.ambient_var: Optional[str] = None
    ...
    def get_trig_rules(self, rule_name: str) -> List[Identity]:
        res = self.parent.get_trig_rules(rule_name) if self.parent is not None else []
        res.extend(self.trig_rules.get(rule_name, []))
        return res
    ...
    def set_ambient_var(self, var: Optional[str]):
        self.ambient_var = var

    def get_ambient_var(self) -> Optional[str]:
        if self.ambient_var is not None:
            return self.ambient_var
        return self.parent.get_ambient_var() if self.parent is not None else None
    ...
    def add_other_identities(self, eq: Expr, category: str, attributes: Optional[List[str]] = None,
                             rule: str = "", const_vars: Optional[List[str]] = None):
        if not eq.is_equals():
            raise TypeError
        
        symb_lhs = expr_to_pattern(eq.lhs, const_vars)
        symb_rhs = expr_to_pattern(eq.rhs, const_vars)
        one_shot = attributes is not None and 'one_shot' in attributes
        self.other_identities.append(Identity(symb_lhs, symb_rhs, category=category, rule=rule,
                                              one_shot=one_shot))
        if rule:
            self.trig_rules.setdefault(rule, []).append(Identity(symb_lhs, symb_rhs, category=category,
                                                                 rule=rule, one_shot=one_shot))
        if attributes is not None and 'bidirectional' in attributes:
            self.other_identities.append(Identity(symb_rhs, symb_lhs, category=category, rule=rule,
                                                  one_shot=one_shot))
        ...
        if item['type'] in ('axiom', 'problem', 'theorem', 'calculation'):
            ...
            if e.is_equals() and e.lhs.is_indefinite_integral():
                ...
                self.add_other_identities(e, item['category'], item.get('attributes'),
                                          item.get('rule', ''), item.get('const_vars'))

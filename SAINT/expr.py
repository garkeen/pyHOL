...
class Expr:
    ...
    def replace_expr(self, loc, new_expr: "Expr") -> "Expr":
        ...
        if loc.is_empty():
            ...
            assert loc.head == 0, "replace_expr: invalid location"
            return IndefiniteIntegral(self.var, self.body.replace_expr(loc.rest, new_expr), self.skolem_args)
        elif self.is_limit():
            assert loc.head == 0, "replace_expr: invalid location"
    ...
    def expand(self) -> "Expr":
        """Expand products of sums and powers with positive integer exponents.

        E.g. ``(a + b) * c`` becomes ``a * c + b * c`` and ``(a + b) ^ 2``
        becomes ``a * a + a * b + b * a + b * b`` (which a later
        normalization combines into ``a ^ 2 + 2 * a * b + b ^ 2``).
        Subexpressions are expanded recursively.

        """
        def rec(e: "Expr") -> "Expr":
            if e.is_plus():
                return Op("+", rec(e.args[0]), rec(e.args[1]))
            elif e.is_uminus():
                return Op("-", rec(e.args[0]))
            elif e.is_minus():
                return Op("-", rec(e.args[0]), rec(e.args[1]))
            elif e.is_times():
                a = rec(e.args[0])
                b = rec(e.args[1])
                if a.is_plus():
                    return rec(Op("+", Op("*", a.args[0], b), Op("*", a.args[1], b)))
                elif a.is_minus():
                    return rec(Op("-", Op("*", a.args[0], b), Op("*", a.args[1], b)))
                elif b.is_plus():
                    return rec(Op("+", Op("*", a, b.args[0]), Op("*", a, b.args[1])))
                elif b.is_minus():
                    return rec(Op("-", Op("*", a, b.args[0]), Op("*", a, b.args[1])))
                else:
                    return Op("*", a, b)
                ...
                base = rec(e.args[0])
                exp = e.args[1]
                if exp.is_const() and isinstance(exp.val, int) and exp.val >= 1:
                    result = base
                    for _ in range(exp.val - 1):
                        result = rec(Op("*", result, base))
                    return result
                else:
                    return Op("^", base, exp)
            elif e.is_op():
                return Op(e.op, *[rec(arg) for arg in e.args])
            elif e.is_fun():
                return Fun(e.func_name, *[rec(arg) for arg in e.args])
            elif e.is_deriv():
                return Deriv(e.var, rec(e.body))
            elif e.is_integral():
                return Integral(e.var, rec(e.lower), rec(e.upper), rec(e.body))
            elif e.is_evalat():
                return EvalAt(e.var, rec(e.lower), rec(e.upper), rec(e.body))
            elif e.is_indefinite_integral():
                return IndefiniteIntegral(e.var, rec(e.body), e.skolem_args)
            elif e.is_limit():
                return Limit(e.var, rec(e.lim), rec(e.body))
            elif e.is_summation():
                return Summation(e.index_var, rec(e.lower), rec(e.upper), rec(e.body))
            else:
                return e
        return rec(self)
    ...
    def replace_pattern(self, pattern: "Expr", repl_e: "Expr") -> "Expr":
        """Replace subexpressions matching pattern, using commutative matching."""
        assert isinstance(pattern, Expr) and isinstance(repl_e, Expr)
        if match(self, pattern) is not None:
            return repl_e
        elif self.ty in (VAR, CONST, INF):
            return self
        elif self.is_op():
            return Op(self.op, *[arg.replace_pattern(pattern, repl_e) for arg in self.args])
        elif self.is_fun():
            return Fun(self.func_name, *[arg.replace_pattern(pattern, repl_e) for arg in self.args])
        elif self.is_deriv():
            return Deriv(self.var, self.body.replace_pattern(pattern, repl_e))
        elif self.is_integral():
            return Integral(self.var, self.lower.replace_pattern(pattern, repl_e),
                            self.upper.replace_pattern(pattern, repl_e),
                            self.body.replace_pattern(pattern, repl_e))
        elif self.is_evalat():
            return EvalAt(self.var, self.lower.replace_pattern(pattern, repl_e),
                          self.upper.replace_pattern(pattern, repl_e),
                          self.body.replace_pattern(pattern, repl_e))
        elif self.is_skolem_func():
            return SkolemFunc(self.name, tuple(var.replace_pattern(pattern, repl_e) for var in self.dependent_vars))
        elif self.is_summation():
            return Summation(self.index_var, self.lower.replace_pattern(pattern, repl_e),
                             self.upper.replace_pattern(pattern, repl_e),
                             self.body.replace_pattern(pattern, repl_e))
        elif self.is_limit():
            return Limit(self.var, self.lim.replace_pattern(pattern, repl_e),
                         self.body.replace_pattern(pattern, repl_e), self.drt)
        else:
            print(self)
            raise NotImplementedError
    ...
    def rec(exp: Expr, pattern: Expr, bd_vars: Dict[str, str]):
        ...
        if exp.is_var():
            ...
            if exp.op != pattern.op or len(exp.args) != len(pattern.args):
                return False
            if len(exp.args) == 2 and exp.op in ('+', '*'):
                # Commutative operators: try both argument orders.
                for a0, a1 in ((exp.args[0], exp.args[1]), (exp.args[1], exp.args[0])):
                    saved = dict(d)
                    if rec(a0, pattern.args[0], bd_vars) and rec(a1, pattern.args[1], bd_vars):
                        return True
                    d.clear()
                    d.update(saved)
                return False
...
def expr_to_pattern(e: Expr, const_vars: Optional[Iterable[str]] = None) -> Expr:
    """Convert an expression to pattern.

    Variables listed in ``const_vars`` are restricted to match constants
    only (used e.g. for rules parameterized by a numeric factor).

    """
    const_vars = set(const_vars or [])
    vars = e.get_vars()
    for var in vars:
        if var in const_vars:
            e = e.subst(var, Symbol(var, [CONST]))
        else:
...
def find_pattern(expr, pat, transform=None):
    ...
    def rec(e, pat, cur_loc):
        ...
        elif e.ty in (INTEGRAL, DERIV, EVAL_AT, INDEFINITEINTEGRAL):

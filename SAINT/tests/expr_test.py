...
from SAINT.expr import Var, Const, Op, Fun, sin, cos, log, exp, Deriv, Integral, EvalAt, Symbol, \
...
class ExprTest(unittest.TestCase):
    ...
    def testNormalizeConstant(self):
        test_data = [
            ("(3 + sqrt(2)) * (2 + sqrt(2))", "5 * sqrt(2) + 8"),
            ("(3 + sqrt(2)) * (2 + sqrt(3))", "sqrt(2) * sqrt(3) + 2 * sqrt(2) + 3 * sqrt(3) + 6"),
            ("sqrt(8)", "2 * sqrt(2)"),
            ...
            ("sqrt(8) / (1 + sqrt(10))", "2 * sqrt(2) * (sqrt(2) * sqrt(5) + 1) ^ (-1)"),
            ("sqrt(8) ^ 2", "8"),
            ("(3 + sqrt(2)) ^ 2", "6 * sqrt(2) + 11"),
            ("(3 + sqrt(2)) ^ 3", "29 * sqrt(2) + 45"),
    ...
    def testNormalize(self):
        test_data = [
            ...
            ("x / sqrt(5 - 4 * x)", "x * (-4 * x + 5) ^ (-1/2)"),
            ("1 / (1 + sqrt(x))", "(sqrt(x) + 1) ^ (-1)"),
            ...
            ("log(exp(2))", "2"),
            ("log(x^2)", "2 * log(x)"),
            ("sqrt(cos(x) - cos(x)^3)", "sqrt(-(cos(x) ^ 3) + cos(x))"),
            # ("sqrt(cos(x) * (1 - cos(x)^2))", "sqrt(cos(x) - cos(x) ^ 3)"),
            ("1/2 * u ^ (-1) * 2 * u", "1"),
            ("1/2 * u ^ (-1) * (2 * u / (1 + u ^ (2)))", "(u ^ 2 + 1) ^ (-1)"),
            ("[log(1 + u ^ 2)]_u=-1,1", "0"),
            # ("sqrt(x ^ 2)", "abs(x)"),
            ("[abs(x)]_x=-2,3", "1"),
            ...
            ("2 * (INT x:[1,exp(1)]. 1) - ([u * log(u)]_u=2,2 * exp(1))",
             "-2 * exp(1) * log(2) - 2 * exp(1) + 2 * log(2) + 2 * (INT x:[1,exp(1)]. 1)"),
            ("3 ^ (1/2) * 3 ^ (1/2)", "3"),
            ("2 * ((1/2) * 3 ^ (1/2))", "sqrt(3)"),
